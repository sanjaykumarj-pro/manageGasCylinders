...
export default function DistrictDashboard() {
    ...
    const handleForward = async (id) => {
        try {
            await axios.post(`/api/complaints/items/${id}/forward_to_commissioner/`, {});
            fetchComplaints();
    ...
    const handleReject = async (id) => {
        try {
            await axios.post(`/api/complaints/items/${id}/reject/`, { comments: 'Rejected by District Officer' });
            fetchComplaints();
    ...
    const stats = {
        ...
        district_pending: complaints.filter(c => c.status === 'UNDER_DISTRICT_REVIEW').length,
        forwarded: complaints.filter(c => c.status === 'FORWARDED_TO_COMMISSIONER' || c.status === 'APPROVED').length,
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="max-w-7xl mx-auto p-6 md:p-10 w-full">
                <header className="mb-10">
                    <h1 className="text-4xl font-black text-gray-900 mb-2">District Supply Officer</h1>
                    <p className="text-gray-500">Monitor and escalate SOS reports for your assigned jurisdiction.</p>
                </header>

                {/* Stats Row */}
                <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-10">
                    <StatCard label="Total SOS" value={stats.total} color="text-gray-900" />
                    <StatCard label="Pending" value={stats.district_pending} color="text-amber-600" />
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                    <h2 className="text-xl font-bold text-gray-900 mb-6">
                        SOS Escalation Queue — {AGENCY_BRANCHES.find(b => b.key === activeBranch)?.label}
                    </h2>
                                        ...
                                        <tr key={c.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="py-4">
                                                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${
                                                    ...
                                                    c.status === 'UNDER_DISTRICT_REVIEW' ? 'bg-amber-100 text-amber-700' :
                                                    c.status === 'FORWARDED_TO_COMMISSIONER' ? 'bg-purple-100 text-purple-700' :
                                            ...
                                            <td className="py-4 flex gap-2">
                                                ...
                                                {c.status === 'UNDER_DISTRICT_REVIEW' && (
                                                    <>
                                                        <button onClick={() => handleForward(c.id)} className="p-2 text-orange-400 hover:text-orange-600" title="Forward to Commissioner">
